 ...
 * @param {boolean} cfg.hasNewComments - should issue be returned with new comments
 * @param {string} cfg.hasCommentsForRole - visible only to groups
 * @param {string} cfg.hasCommentsWithVisibility - internal or external
...
async function processTrigger(msg, cfg, snp) {
    ...
    if(cfg.hasCommentsForRole) {
        commentedFuncArgs.push('"role ' + jira.jqlEncode(cfg.hasCommentsForRole, true) + '"');
    }

    if(cfg.hasCommentsWithVisibility) {
        commentedFuncArgs.push('"visibility ' + jira.jqlEncode(cfg.hasCommentsWithVisibility, true) + '"');
    }

    if(cfg.hasNewComments && (snp.updated || cfg.startDateTime)) {
        commentedFuncArgs.push('"after ' + "'" + jira.jqlDate(snp.updated) || cfg.startDateTime + "'" + '"');
    }
