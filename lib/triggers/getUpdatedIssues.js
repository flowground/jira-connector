...
function processTrigger(msg, cfg, snp) {
    ...
    return jira.searchIssues(jql, {
        ...
        let issues = res.issues.filter(issue => issue.updated > dateTime || (issue.updated === dateTime && Number(issue.id) > snp.lastProcessedIssueId ));
        issues.forEach(issue => {
            let dateTime = issue.updated.slice(0, 16).replace('T', ' ');
