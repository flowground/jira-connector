 ...
 * @param {boolean} cfg.withNewComments - should issue be returned with new comments
 * @param {string} cfg.withCommentsForRole - visible only to groups
 * @param {string} cfg.withCommentsWithVisibility - internal or external
...
async function processTrigger(msg, cfg, snp) {
    ...
    if(cfg.withCommentsForRole) {
        commentedFunctArgs.push('"role ' + "'" + this.escapeQuotes(cfg.withCommentsForRole) + "'" + '"');
    }

    if(cfg.withCommentsWithVisibility) {
        commentedFunctArgs.push('"visibility ' + "'" + this.escapeQuotes(cfg.withCommentsWithVisibility) + "'" + '"');
    }

    if(cfg.withNewComments && (snp.updated || cfg.startDateTime)) {
        commentedFunctArgs.push('"after ' + "'" + jira.jqlDate(snp.updated) || cfg.startDateTime + "'" + '"');
    }
