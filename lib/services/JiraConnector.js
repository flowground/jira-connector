...
class JiraConnector {
     ...
     * @param {object} input.fields.otherfields - other Jira fields (including custom fields)
     * @returns {object} - Jira issue object
     */
    mapInputToIssue(input) {
        let issue = JSON.parse(JSON.stringify(input));

        if('otherfields' in issue.fields) {
            Object.assign(issue.fields, issue.fields.otherfields);
            delete issue.fields.otherfields;
        }
