...
function eioProcess(process) {
    return function(msg, cfg, snapshot) {
        console.log('---PROCESS body 100c: %s', (JSON.stringify(msg.body) || '').slice(0, 100));
        console.log('---PROCESS body keys: %j', Object.keys(msg.body));
        console.log('---PROCESS pass keys: %j', Object.keys(msg.passthrough));
