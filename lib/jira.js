...
//
// [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id290709)
this.getIssueCreateMeta = function(projectId, issueTypeId, callback) {

    var options = {
        rejectUnauthorized: this.strictSSL,
        uri: this.makeUri(`/issue/createmeta?projectIds=${projectId}&issuetypeIds=${issueTypeId}&expand=projects.issuetypes.fields`),
        method: 'GET'
    };
    ...
    this.doRequest(options, function(error, response, body) {
        ...
        if (response.statusCode === 404) {
            callback('Invalid projectId or issueTypeId.');
            return;
        }

        if (response.statusCode !== 200) {
            callback(response.statusCode + ': Unable to connect to JIRA during getIssueCreateMeta.');
            return;
        }
